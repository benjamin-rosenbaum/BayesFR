...
#' solution of the (generalized) Beddington-DeAngelis model
...
#' \verb{~ TypeBD_dyn(N, P, Time, b, h, q, c)}\cr\cr
...
#' \verb{~ TypeBD_dyn(N, P, Time, a, h, 0, c)}\cr\cr
#' Type 3 functional response: \cr\cr
#' \verb{~ TypeBD_dyn(N, P, Time, b, h, 1, c)}

#' @return Number of eaten prey
#' @export
TypeBD_dyn_code = "
// ODE function
vector TypeBDODE(real t, vector y, vector theta){
  vector[1] dydt;
...
// prediction
real TypeBD_dyn(real N, real P, real Time, real b, real h, real q, real c){
  array[1] vector[1]  y;
  vector[5] theta;
  theta[1] = b;
  theta[2] = h;
  ...
  theta[4] = P;
  theta[5] = c;
  y = ode_rk45(TypeBDODE, [N]', 0.0, {Time}, theta);
  return(N-y[1][1]);
}
"
