...
#' solution of the \bold{(generalized) Crowley-Martin} model
...
#' \verb{~ TypeGenCM_dyn(N, P, Time, b, h, q, c)}\cr\cr
...
#' \verb{~ TypeGenCM_dyn(N, P, Time, a, h, 0, c)}\cr\cr
#' Type 3 functional response: \cr\cr
#' \verb{~ TypeGenCM_dyn(N, P, Time, b, h, 1, c)}
...
// prediction
real TypeGenCM_dyn(real N, real P, real Time, real b, real h, real q, real c){
  array[1] vector[1]  y;
  vector[5] theta;
  theta[1] = b;
  theta[2] = h;
  ...
  theta[4] = P;
  theta[5] = c;
  y = ode_rk45(TypeGenCMODE, [N]', 0.0, {Time}, theta);
  return(N-y[1][1]);
}
"
