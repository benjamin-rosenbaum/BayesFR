...
#' solution of the (generalized) Crowley-Martin model
#' \deqn{\frac{dN}{dt}=-\frac{bN^{1+q}}{(1+bhN^{1+q})(1+c(P-1))}P}{dN/dt=-bN^(1+q)P/((1+bhN^{1+q})(1+c(P-1)))}
...
#' \verb{~ TypeCM_dyn(N, P, Time, b, h, q, c)}\cr\cr
...
#' \verb{~ TypeCM_dyn(N, P, Time, a, h, 0, c)}\cr\cr
#' Type 3 functional response: \cr\cr
#' \verb{~ TypeCM_dyn(N, P, Time, b, h, 1, c)}

#' @return Number of eaten prey
#' @export
TypeCM_dyn_code = "
// ODE function
vector TypeCMODE(real t, vector y, vector theta){
  vector[1] dydt;
  real bNq = theta[1]*y[1]^theta[3];
  if(y[1]<1e-8){
    dydt[1] = 0.0;
  } else {
    dydt[1] = -bNq*theta[4] / ( (1.0+theta[2]*bNq) * (1.0+theta[5]*(theta[4]-1.0)) );
  }
  return dydt;
}
// prediction
real TypeCM_dyn(real N, real P, real Time, real b, real h, real q, real c){
  array[1] vector[1]  y;
  vector[5] theta;
  theta[1] = b;
  theta[2] = h;
  ...
  theta[4] = P;
  theta[5] = c;
  y = ode_rk45(TypeCMODE, [N]', 0.0, {Time}, theta);
  return(N-y[1][1]);
}
"
